...
async def tcp_client(p, encrypted, key, a, loop, len):
...
def sendfile(filename, p):
    ...
    loop.run_until_complete(tcp_client(p, encrypted, key, a, loop, datalen))
    loop.close()


if __name__ == '__main__':
    from params import *
    sendfile("opentext.txt", int(r, 16))
